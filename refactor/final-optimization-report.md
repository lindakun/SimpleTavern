...
| P0-2: Repository 反向依赖 | ✅ 完成 | 1 处未使用导入 | 0 处 |
...
| P1-7: 统一错误响应构造 | ✅ 完成 | 20+ 处手动响应 → AppError |
| P1-10: multer 错误处理 | ✅ 完成 | 区分 MulterError 类型 |
| P1-11: 修复 HTTP 泄漏 | ✅ 完成 | 删除未使用的 getRequestContext 导入 |
...
- `backend/src/modules/auth/auth.controller.ts` - 10 处 Session 类型 + logout 修复 + recoveryCodes TTL + 20+ 处 AppError
...
- `backend/src/modules/users/users.repository.ts` - 删除未使用导入 + 6 处 catch 注释
...
- `backend/src/modules/worlds/admin-worlds.controller.ts` - multer 错误处理改进
...
### P1 剩余（6 项）
- **P1-1**: 角色卡元数据 LRU 缓存（性能优化）
- **P1-2**: 消息气泡 React.memo 优化（前端性能）
- **P1-3**: JSONL 聊天分页加载（大文件处理）
- **P1-4**: 虚拟滚动 overscan 优化（前端性能）
- **P1-5**: 角色卡缩略图生成（图片优化）
